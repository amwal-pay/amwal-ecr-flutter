...
and an `ecrMode` of `1` (USB cable, **Android only**), `2` (wi‑fi), or `4`
(Web Service). A terminal in wi‑fi mode listens on port 9100; USB cable has no
IP. On Windows and iOS, USB cable is typed unsupported — use wi‑fi or Web
...
allow the app through the firewall for outbound TCP (and HTTPS for Web Service).
...
- [ ] On Windows, use Wi‑Fi or Web Service only (USB cable is unsupported), and
      confirm the PC can route to the terminal / Hub.
